...
class block_fesigninsheet extends block_base {
...
function init() {
    ...
    $this->title   = get_string('pluginname', 'block_fesigninsheet');
    $plugin = new stdClass();
  }
...
function get_content() {
	...
	$blockhidden = get_config('block_fesigninsheet', 'hidefromstudents');

	//
	// If the admin has selected to hide from students
	//
	if (!empty($blockhidden)) {
		if (has_capability('block/fesigninsheet:viewblock', $this->context)) {
...
function getSignInNav(){
	...
	$bodyhtml = '<img src="'.$CFG->wwwroot. '/blocks/fesigninsheet/printer.gif"/> <a href="'.$CFG->wwwroot. '/blocks/fesigninsheet/genlist/show.php?cid='.$cid.'">'. get_string('genlist', 'block_fesigninsheet').'</a><br>
