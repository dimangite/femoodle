...
// Setup the page
$PAGE->set_title(get_string('uploadimage', 'block_fesigninsheet'));
$PAGE->set_heading(get_string('uploadimage', 'block_fesigninsheet'));
...
file_prepare_draft_area($draftitemid, $context->id, 'block_fesigninsheet', 'attachment', $itemid, $filemanageropts);
...
if ($mform->is_cancelled()) {
    ...
    file_save_draft_area_files($draftitemid, $context->id, 'block_fesigninsheet', 'attachment', $itemid, $filemanageropts);



$fs = get_file_storage();
if ($files = $fs->get_area_files($context->id, 'block_fesigninsheet', 'attachment', '0', 'sortorder', false)) {
...
$recordexists = $DB->get_record('block_fesigninsheet', array('id'=>1));
    ...
    if($recordexists){
            ...
            $DB->update_record('block_fesigninsheet', $record, $bulk=false);
    } else {
                    $record = new stdClass();
                    $record->fieldvarname = 'imgurl';
                    $record->field_value = $cleanPath;
                    $DB->insert_record('block_fesigninsheet', $record, false);
    }
        

    echo '<script>window.location="../../admin/settings.php?section=blocksettingfesigninsheet";</script>';
