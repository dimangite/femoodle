 ...
 * 
 * Description:
 * This is the main display page used for calling each different representation
 * of signinsheets
 * ----------------------------------------------------------------------
 */
...
require_once('renderfesigninsheet.php');
...
if($rendertype == 'all' || $rendertype == ''){
		...
		$PAGE->navbar->add(get_string('showall', 'block_fesigninsheet'));
	
}
else if($rendertype == 'group'){
		$courseName = $DB->get_record('course', array('id'=>$cid), 'shortname', $strictness=IGNORE_MISSING); 
		$PAGE->navbar->add($courseName->shortname, new moodle_url($CFG->wwwroot . '/course/view.php?id=' . $cid));
		$PAGE->navbar->add(get_string('showbygroup', 'block_fesigninsheet'));
}


$PAGE->set_url('/blocks/fesigninsheet/showfesigninsheet/show.php');
$PAGE->set_context(get_system_context());
$PAGE->set_heading(get_string('pluginname', 'block_fesigninsheet'));
$PAGE->set_title(get_string('pluginname', 'block_fesigninsheet'));
...
$logoenabled = get_config('block_fesigninsheet', 'customlogoenabled');
...
class fesigninsheet_form extends moodleform {
 ...
 * hand side of the feshowsigninsheet.php page
 * 
 * */
function buildMenu($cid){
	...
	$outputhtml = '<div style="float:right"><form action="'.$CFG->wwwroot. '/blocks/fesigninsheet/genlist/show.php?cid='.$cid.'" method="post">
				 '.get_string('orderby', 'block_fesigninsheet').': <select name="orderby" id="orderby">
								<option value="firstname">' .get_string('firstname', 'block_fesigninsheet').'</option>
								<option value="lastname" selected>'.get_string('lastname', 'block_fesigninsheet').'</option>
						  </select>
						  
				 '.get_string('filter', 'block_fesigninsheet').': <select id="selectgroupsec" name="selectgroupsec">
				 	<option value="all">'.get_string('showall', 'block_fesigninsheet').'</option>
				 '. buildGroups($cid).'	
				 </select>
				 <input type="submit" value="'.get_string('update', 'block_fesigninsheet').'"></input>
				</form>
				...
				'.get_string('blankfields', 'block_fesigninsheet').': <input type="number" min="0" size="3" name="extra" value="0">
	...
	$outputhtml .= '
				...
				<input type="hidden" name="orderby" value="'.$orderby.'">
   				...
   				<input type="submit" value="'.get_string('printbutton', 'block_fesigninsheet').'">
				</span>
				</form>
