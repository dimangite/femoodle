...
function printHeaderLogo(){
	...
	global $DB;
 	...
 	$imageurl =  $DB->get_field('block_fesigninsheet', 'field_value', array('id'=>1), $strictness=IGNORE_MISSING);
...
function renderGroup($extra){
	...
	$addfieldenabled = get_config('block_fesigninsheet', 'includecustomfield');
    ...
    $outputhtml .= '<div style="text-align: center"><span style="font-size:20px"> <b>'. get_string('course', 'block_fesigninsheet').': </b>' .$courseSName->shortname.' - ' .$courseName->fullname.'</span></div><p></p>';

    $reportdate = date('l jS \of F Y');
    $outputhtml .= '<div style="text-align: center"><span style="font-size:18px"> <b>'. get_string('date', 'block_fesigninsheet').':</b> '.$reportdate.'</span></div><p></p>';
	...
	$outputhtml .= '<table style="border-style: solid; border-collapse: collapse;" width="600px"  border="1px"><tr>
					<td style="border-right: thin solid; border-bottom: thin solid" border="1px" width="200"><b>'. get_string('personname', 'block_fesigninsheet').'</b></td>
				';
	

	
if($addfieldenabled){
		$fieldid = get_config('block_fesigninsheet', 'customfieldselect');
...
$addtextfield = get_config('block_fesigninsheet', 'includecustomtextfield');
if($addtextfield){
		$fielddata = get_config('block_fesigninsheet', 'customtext');
...
$addidfield = get_config('block_fesigninsheet', 'includeidfield');
if($addidfield){
		
		$outputhtml.='<td style="border-right: thin solid; border-bottom: thin solid" border="1px" width="150"><b>'. get_string('idnumber', 'block_fesigninsheet').' </b></td>';

}	

// Add additional mdl_user field if enabled
$addUserField = get_config('block_fesigninsheet', 'includedefaultfield');
if($addUserField){
	$mdlUserFieldName = get_config('block_fesigninsheet', 'defaultfieldselection');
	$outputhtml.='<td style="border-right: thin solid; border-bottom: thin solid" border="1px" width="150"><b>'. $mdlUserFieldName.' </b></td>';
}
	
	$outputhtml .= '<td style="border-right: thin solid; border-bottom: thin solid" border="1px"><b>'. get_string('signature', 'block_fesigninsheet').'</b></td></tr>';
...
function renderAll($extra){
    ...
    // add Name
    $outputhtml .= '<td style="border-right: thin solid; border-bottom: thin solid" width="200"><b>'. get_string('personname', 'block_fesigninsheet').'</b></td>';
    ...
    $addUserField = get_config('block_fesigninsheet', 'includedefaultfield');
    if($addUserField){
        $mdlFieldName = get_config('block_fesigninsheet', 'defaultfieldselection');
        $outputhtml.='<td style="border-right: thin solid; border-bottom: thin solid" border="1px" width="70"><b>'. $mdlFieldName.' </b></td>';
    }


	$outputhtml .='	<td style="border-right: thin solid; border-bottom: thin solid" border="1px"><b>'.get_string('signature', 'block_fesigninsheet').'</b></td>
	</tr>';
...
function printblank(){
	...
	$outputhtml =  '<tr height="10"><td  style="border-right: thin solid;  border-bottom: thin solid" border="1px" width="50" height="30"> &nbsp;</td>';
    ...
    // Include additional field data if enabled
	if($addfieldenabled){
		$outputhtml .=	'<td style="border-right: thin solid; border-bottom: thin solid" border="1px" width="150" height="30">&nbsp; </td>';
	} 
