...
require_once 'grade_export_fexls.php';

$id                = required_param('id', PARAM_INT); // course id
$PAGE->set_url('/grade/export/fexls/export.php', array('id'=>$id));
...
require_capability('gradeexport/fexls:view', $context);
...
if (!empty($CFG->gradepublishing) && !empty($key)) {
    print_grade_page_head($COURSE->id, 'export', 'fexls', get_string('exportto', 'grades') . ' ' . get_string('pluginname', 'gradeexport_fexls'));
}
...
$export = new grade_export_fexls($course, $groupid, $formdata);
...
if (!empty($CFG->gradepublishing) && !empty($key)) {
    ...
    $event = \gradeexport_fexls\event\grade_exported::create(array('context' => $context));
    $event->trigger();
    $export->print_grades();
}
